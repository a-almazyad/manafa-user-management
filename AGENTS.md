...
- UC-008 must enhance, not replace, the existing pending-request behavior: pending Authorization requests retain permission configuration plus approve/reject actions; pending Delegation requests retain delegator selection plus approve/reject actions; request details include requester NID/Iqama, DOB, phone number, request date, and attached documents.

...
- In UC-001 Current Structure, place the read-only Owners & Executives table before the ownership hierarchy. Its columns are exactly `Name`, `ID`, `Role`, and `Ownership Percentage`.
- Keep the UC-001 hierarchy intentionally lightweight: each owner row shows only the RTL owner name and ownership percentage. Show hierarchy through expandable nested levels and subtle level styling, applying indentation to the row controls instead of the Arabic text.
- For each corporate owner in the UC-001 hierarchy, show a visible `Retrieve Ownership` button. It must confirm the Wathq call and insert the returned owners below that company; keep cooldown and error protection without adding dense metadata columns to the hierarchy.
...
- The UC-001 hierarchy must support multiple ownership levels, per-level expand/collapse, clickable individual and corporate owner names, explicit corporate-owner Wathq retrieval with confirmation, automatic expansion after a successful retrieval, and the existing cooldown/error protections.
...
- Keep compact icon-only actions for existing admin tables that need an Actions column, but do not add an Actions column to the four-column UC-001 Current Structure table.
